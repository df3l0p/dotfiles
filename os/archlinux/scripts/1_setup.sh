...
# Author: Boogy
# Description:
#   This script installs and configure archlinux
# Source: 
# - https://github.com/boogy/dotfiles/blob/master/deploy/archlinux/sysprep_archlinux.sh
#
############################################
...
cp ./sysprep_arch.sh /mnt
chmod +x /mnt/sysprep_arch.sh


## Continue the installation from the chroot
##
msg_ok "Swithing to arch-chroot in /mnt"
arch-chroot /mnt chmod +x /sysprep_arch.sh
arch-chroot /mnt ./sysprep_arch.sh ${PARTITION2} ${SHOULD_ENCRYPT_DISK} ${PARTITION_LUKS}