...
# Author: Boogy
# Description:
#   This script installs and configure archlinux from a booted archlinux iso device
# WARNING:
...
# Source: 
# - https://github.com/boogy/dotfiles/blob/master/deploy/archlinux/sysprep_archlinux_chroot.sh
#
############################################
...
HOSTNAME=${tmp_HOSTNAME:-"mob-arc-dfelop"}
USERNAME=${tmp_USERNAME:-"dfelop"}
...
{
    local DEFAULT_PACKAGES_LIST=./packages.txt
    if [[ -f ${DEFAULT_PACKAGES_LIST} ]]; then
        ...
        curl -q https://raw.githubusercontent.com/df3l0p/setupfiles/master/archlinux/packages/packages.conf --output ./packages.txt
