# Very suspicious behavior with auto-suggestion
###
# If you have issues with auto-suggestion not using a gray color
# Copy paste the following entry in .zshrc
# ZSH_AUTOSUGGEST_HIGHLIGHT_STYLE="fg=#ff00ff,bg=cyan,bold,underline"
# Then, load it with source. After that remove the line again and normaly it should work
###
...
alias impacket="docker run --rm -it custom/impacket"

smbservehere() {
    local sharename
    [[ -z $1 ]] && sharename="SHARE" || sharename=$1
    docker run --rm -it -p 445:445 -v "${PWD}:/tmp/serve" custom/impacket smbserver.py -smb2support $sharename /tmp/serve
}

alias nginxhere='docker run --rm -it -p 80:80 -p 443:443 -v "${PWD}:/srv/data" custom/nginxserve'
